...
class TaggedMembersModel {
  ...
  factory TaggedMembersModel.fromJson(Map<String, dynamic> json) {
    return TaggedMembersModel(
      ...
      taggedMembersList: List<TaggedMember>.from(json['taggedMembersList'].map(
          (member) => TaggedMember.fromJson(
              member))), // JSON에서 'taggedMembersList' 리스트를 변환하여 할당
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'taggedMembersList': List<dynamic>.from(taggedMembersList.map(
          (member) => member.toJson())), // 'taggedMembersList'를 JSON 리스트로 변환
    };
  }
}
